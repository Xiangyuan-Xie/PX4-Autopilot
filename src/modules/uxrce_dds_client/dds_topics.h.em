...
@[    for idx, sub in enumerate(subscriptions + subscriptions_multi)]@
	{
			...
			create_data_reader(session, reliable_out_stream_id, @('reliable_in_stream_id' if sub.get('reliable', False) else 'best_effort_in_stream_id'),
					   participant_id, @(idx), client_namespace, "@(sub['topic'])", message_version,
					   "@(sub['dds_type'])", @('true' if sub.get('reliable', False) else 'false'), queue_depth);
	}
@[    end for]@
