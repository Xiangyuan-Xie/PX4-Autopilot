...
TEST_F(LoggedTopicsTest, DefaultProfileLogsAmTopics)
{
	...
	EXPECT_TRUE(topics.has_subscription(ORB_ID(am_pose_control_status)));
	EXPECT_TRUE(topics.has_subscription(ORB_ID(am_pose_policy_observation)));
	EXPECT_TRUE(topics.has_subscription(ORB_ID(am_pose_trajectory)));
